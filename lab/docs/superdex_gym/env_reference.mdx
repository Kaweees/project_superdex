...
shipped environment contracts, see the [Gallery of Environments](./envs/overview.mdx).
...
List every SuperDex environment registered in this installation -- these canonical IDs are exactly what the CLI accepts:

```python
from superdex.lab.gym.registration import get_env_specs, register_envs

register_envs()  # idempotent; registers the public SuperDex specs with Gymnasium
for spec in sorted(get_env_specs(), key=lambda spec: spec.id):
    print(spec.id)
```
